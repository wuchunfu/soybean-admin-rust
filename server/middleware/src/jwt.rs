...
mod tests {
    ...
    async fn user_info_handler(user: User) -> impl IntoResponse {
        Res::new_success(user, StatusCode::UNAUTHORIZED.as_str()).into_response()
    }
    ...
    async fn test_user_info_endpoint() {
        ...
        let app = Router::new().route("/user_info", get(user_info_handler)).layer(
            axum::middleware::from_fn(move |req, next| jwt_auth_middleware(req, next, "audience")),
        );
